...
- Public receipts, deployment input, state, events, signatures, caller reward
  and treasury withdrawals, and observed enforcement gas coverage can be
  recomputed through an explicitly supplied independent archive RPC endpoint.
- No external package-size or invocation bound is used.
- Unslashed owner-funded bonds cannot be returned before the latest release
  time; an irreversible retirement path returns them afterward.
...
- The deployer funds every covered bond. The experiment therefore establishes
  covered on-chain bond loss, not independently borne operator loss.
