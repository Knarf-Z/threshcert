...
/// @notice Trace-Then-Slash mechanism for a fixed 4-of-7 committee.
...
contract TraceThenSlash is EIP712, Ownable, ReentrancyGuard {
    ...
    bool public committeeRetired;
    uint8 public registeredCount;
    uint64 public jobCount;
    uint64 public latestReleaseTime;
    ...
    event RemainingBondsWithdrawn(address indexed recipient, uint256 amount);
    ...
    function openRelease(
        ...
        if (committeeRetired) revert CommitteeAlreadyRetired();
        if (releaseTime <= block.timestamp) revert InvalidReleaseTime();
        uint64 nonce = ++jobCount;
        if (releaseTime > latestReleaseTime) latestReleaseTime = releaseTime;
    ...
    /// @notice Retires the controlled committee and returns every unslashed bond.
    /// @dev Registration is owner-funded in this scoped mechanism, so the owner
    ///      selects the refund recipient. Retirement is irreversible and is
    ///      available only after every opened release window has ended.
    function withdrawRemainingBonds(
        address payable recipient
    ) external onlyOwner nonReentrant {
        if (recipient == address(0)) revert InvalidAddress();
        if (!committeeFrozen) revert CommitteeNotFrozen();
        if (committeeRetired) revert CommitteeAlreadyRetired();
        if (jobCount == 0) revert NoReleaseJobs();
        if (block.timestamp < latestReleaseTime) revert ReleaseWindowStillActive();
        ...
        if (amount == 0) revert NothingToWithdraw();
        committeeRetired = true;

        (bool sent,) = recipient.call{value: amount}("");
        if (!sent) revert TransferFailed();
        emit RemainingBondsWithdrawn(recipient, amount);
    }
