...
const CALLER_REWARD = parseEther(
  process.env.PHASE2_CALLER_REWARD_NATIVE ?? "0.0005",
);
...
if (CALLER_REWARD === 0n || CALLER_REWARD >= BOND) {
  throw new Error(
    "PHASE2_CALLER_REWARD_NATIVE must be positive and smaller than the per-member bond.",
  );
}
...
async function runScenario(mode: Mode) {
  ...
  const treasuryAccrualBeforeWithdrawal = await readContract<bigint>(
    contractAddress,
    "treasuryAccrued",
  );
  const realizedCoveredBondLoss = initialTotalBond - remainingBond;
  ...
  const enforcementGasCost = [...slashTransactions, withdrawal].reduce(
    (total, transaction) => total + BigInt(transaction.gasCostWei),
    0n,
  );
  const rewardCoversEnforcementGas =
    callerRewardBeforeWithdrawal >= enforcementGasCost;
  const netCallerSurplus = rewardCoversEnforcementGas
    ? callerRewardBeforeWithdrawal - enforcementGasCost
    : 0n;

  const treasuryWithdrawal = await writeContract(
    contractAddress,
    "withdrawTreasury",
    [ownerAddress],
    `${mode}:withdraw-treasury`,
  );
  transactions.push(treasuryWithdrawal);
  const treasuryAccrualAfterWithdrawal = await readContract<bigint>(
    contractAddress,
    "treasuryAccrued",
  );
  ...
  return {
    ...
    realizedCoveredBondLossWei: realizedCoveredBondLoss.toString(),
    ...
    treasuryAccruedBeforeWithdrawalWei:
      treasuryAccrualBeforeWithdrawal.toString(),
    treasuryAccruedAfterWithdrawalWei:
      treasuryAccrualAfterWithdrawal.toString(),
    remainingBondWei: remainingBond.toString(),
    postAttackCurrentCertificateWei: postAttackCertificate.toString(),
    enforcementGasCostWei: enforcementGasCost.toString(),
    rewardCoversEnforcementGas,
    netCallerSurplusWei: netCallerSurplus.toString(),
    slashTransactionHashes: slashTransactions.map((item) => item.hash),
    rewardWithdrawalTransactionHash: withdrawal.hash,
    treasuryWithdrawalTransactionHash: treasuryWithdrawal.hash,
    snapshotBlockNumber: treasuryWithdrawal.blockNumber,
    transactions,
  };
}
...
const rewardCoveragePassed = scenarios.every(
  (scenario) => scenario.rewardCoversEnforcementGas,
);
const result = {
  schema: "fc-trace-then-slash-phase2-chiado-v2",
  generatedAt: new Date().toISOString(),
  status: rewardCoveragePassed
    ? "PUBLIC_SCOPED_EXECUTION"
    : "PUBLIC_SCOPED_EXECUTION_REWARD_UNDERFUNDED",
  network: {
    name: "Chiado",
    chainId,
    confirmations: CONFIRMATIONS,
  ...
  committee: {
    ...
    callerRewardWeiPerSlash: CALLER_REWARD.toString(),
    callerRewardFractionBps:
      ((CALLER_REWARD * 10_000n) / BOND).toString(),
...
if (!rewardCoveragePassed) {
  throw new Error(
    "PHASE2_REWARD_GAS_COVERAGE=FAIL; increase PHASE2_CALLER_REWARD_NATIVE and rerun as a new public instance.",
  );
}
