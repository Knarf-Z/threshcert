...
describe("TraceThenSlash mechanism", function () {
  ...
  it("lets only the configured treasury withdraw the exact accrued penalty", async function () {
    const fixture = await setup();
    const { jobId } = await openJob(fixture);
    const evidence = await Promise.all(
      [0, 1, 2, 3].map((index) => makeEvidence(fixture, jobId, index)),
    ...
    await assert.rejects(
      fixture.contract.write.withdrawTreasury(
        [fixture.rewardRecipient.account.address],
    ...
    await fixture.contract.write.withdrawTreasury(
      [fixture.rewardRecipient.account.address],
    ...
    assert.equal(after - before, parseEther("7.6"));
    assert.equal(await fixture.contract.read.treasuryAccrued(), 0n);
  });

  it("returns unslashed owner-funded bonds only after all release windows end", async function () {
    const fixture = await setup();
    const firstJob = await openJob(fixture, 120n);
    const secondJob = await openJob(fixture, 240n);
    const evidence = await Promise.all(
      [0, 1, 2, 3].map((index) =>
        makeEvidence(fixture, firstJob.jobId, index)
      ),
    );
    await fixture.contract.write.slashPackage(packageArgs(evidence), {
      account: fixture.submitter.account,
    ...
    await assert.rejects(
      fixture.contract.write.withdrawRemainingBonds(
        [fixture.rewardRecipient.account.address],
        { account: fixture.owner.account },
      ),
      /ReleaseWindowStillActive/,
    );

    await fixture.networkHelpers.time.increaseTo(firstJob.releaseTime);
    await assert.rejects(
      fixture.contract.write.withdrawRemainingBonds(
        [fixture.rewardRecipient.account.address],
        { account: fixture.owner.account },
    ...
    await fixture.networkHelpers.time.increaseTo(secondJob.releaseTime);
    const before = await fixture.publicClient.getBalance({
      address: fixture.rewardRecipient.account.address,
    });
    await fixture.contract.write.withdrawRemainingBonds(
      [fixture.rewardRecipient.account.address],
      { account: fixture.owner.account },
    );
    const after = await fixture.publicClient.getBalance({
    ...
    assert.equal(await fixture.contract.read.committeeRetired(), true);
    await assert.rejects(openJob(fixture), /CommitteeAlreadyRetired/);
    await assert.rejects(
      fixture.contract.write.withdrawRemainingBonds(
        [fixture.rewardRecipient.account.address],
        { account: fixture.owner.account },
      ),
      /CommitteeAlreadyRetired/,
    );
  });
});
