...
| common_solvency_separation.py | Independently enumerates every financeable sequential checkpoint and atomic package under P <= Lambda + K_Lambda(A) for Lambda in {0,1/2,1,5,100} and k in {1,2,3,5,8}. It checks the exact ((k c + 2 q), 2 q) values, normalized/nonnegative/inclusion-monotone collateral, zero checkpoint slack, the closed-form maximum offer caps, cap-level exposure sufficiency, and cap monotonicity; it also rejects the superseded two-branch collateral formula because its core cap decreases at positive liquidity. Lambda=0 recovers (k+4,4). | LIQUIDITY_ROBUST_SEPARATION=PASS |
