...
| `common_solvency_separation.py` | Independently enumerates every financeable sequential checkpoint and every atomic package under the paper's single collateral function `K(A)` for k=1,2,3,5,8. It derives member acceptance from the same settlement utility in both languages and checks the exact `(k+4, 4)` values. | `COMMON_SOLVENCY_SEPARATION=PASS` |
...
- `common_solvency_separation.py`: exact state-space checks passed for
  k=1,2,3,5,8; sequential values k+4 and package value 4 in every case.
