...
python .\common_solvency_separation.py
python .\refinement_quantifier_boundaries.py
...
| `reproduce_paper_numbers.py` | Reproduces 32 named values: equal-cost weighted activation `TC=2, AC=4`, fixed-uniform-cost mechanism separation `k+2` versus `2`, the exact defensive-allocation table, and baseline values `0/4/18/40`. | `TOTAL CHECKS: 32   MATCH: 32   MISMATCH: 0` |
| `hardening_and_greedy.py` | Reconstructs the exact appendix instances for the coordinated-hardening / pure-M枚bius-interaction theorem and the singleton-greedy-no-guarantee proposition, and checks them **exhaustively** (every subset, not a sample) for several k and M / rho values. | Two `PASS` lines |
...
| `evidence_optimal_atomic_bypass.py` | Verifies the evidence-optimal one-shot atomic-bypass certificate. On 800 random floor ledgers, cap-realizable exact-floor witnesses attain `ABC-bar_b`, while epsilon-raised witnesses approach non-realizable zero-resistance/positive-gate vectors within the proved bound; 800 further cap-realizable perturbations never beat the certificate. Public-only and resistance-only boundaries are checked on 240 and 600 instances, and a fixed strict-infimum ledger is checked at 12 `(b, epsilon)` points. | `tightness_mismatches=0`, `soundness_violations=0`, `strict_infimum_mismatches=0` |
| `mixed_evidence_atomic_bypass.py` | Verifies the partial-evidence master certificate `MABC_{b,M}`. The mixed-tau MCR reduction matches independent brute force on 500 instances; 1,200 random `(b,M)` instances check exact attainment when cap-realizable and epsilon convergence otherwise; 1,200 cap-realizable profiles above the same partial floors never beat the certificate. A fixed strict-infimum ledger is checked at 12 `(b, epsilon)` points. | `mcr_formula_mismatches=0`, `tightness_mismatches=0`, `soundness_violations=0`, `strict_infimum_mismatches=0` |
...
- `refinement_quantifier_boundaries.py` is a finite logical regression and
  countermodel suite; it validates theorem transcription and necessity of the
  added premises, not the general theorem itself.
- The Gnosis production audit and the Chiado testnet pilot are on-chain facts,
  not algorithms -- nothing here re-audits them.
- `common_solvency_separation.py` exhausts representative finite instances of
  the exact shared-utility/shared-collateral construction pinned in the
  appendix. It is a computational transcription check, not a substitute for
  the all-k analytical proof.

## Result summary (as last run)

- `test_equivalence.py`: 720 random instances (n=3..8) + 5 edge cases, 725 total, 0 mismatches.
- `reproduce_paper_numbers.py`: 32/32 matched.
- `refinement_quantifier_boundaries.py`: the trace-product and partition-
  invariance fixtures passed; omitted-terminal and omitted-outcome countermodels
  were rejected; the empty attainable-relation branch passed.
- `common_solvency_separation.py`: 25 exact state-space checks passed for
  Lambda in {0,1/2,1,5,100} and k in {1,2,3,5,8}; every instance matches
  sequential k(Lambda+1)+2(Lambda+2), package 2(Lambda+2), normalized and
  inclusion-monotone collateral, and zero checkpoint
  slack, cap-level exposure sufficiency, exact cap formulas, and cap
  monotonicity; the legacy two-branch formula is negatively rejected.
...
- `atomic_bypass_hierarchy.py`: 640 random instances (n=3..6, b=0..3), 0
  ...
  reproduced exactly (`4,2,2,2,2,2,2,2`). All exact (Fraction arithmetic),
...
- `evidence_optimal_atomic_bypass.py`: 800 random evidence ledgers (n=3..7),
  0 tightness mismatches across exact cap-realizable witnesses and bounded
  epsilon approaches; 800 perturbed cap-realizable profiles, 0 soundness
  violations; 240 public-only and 600 resistance-only instances, 0
  mismatches; 12 fixed strict-infimum `(b, epsilon)` checks, 0 mismatches.
  2,452 total checks, all exact (Fraction arithmetic) and deterministically
  seeded.
- `mixed_evidence_atomic_bypass.py`: 500 mixed-tau MCR comparisons against
  independent brute force, 0 mismatches; 1,200 random `(b, M)` tightness
  checks covering exact attainment and bounded epsilon approaches, 0
  mismatches; 1,200 cap-realizable soundness profiles, 0 violations; 12
  fixed strict-infimum `(b, epsilon)` checks, 0 mismatches. 2,912 total
  checks, all exact (Fraction arithmetic) and deterministically seeded.
