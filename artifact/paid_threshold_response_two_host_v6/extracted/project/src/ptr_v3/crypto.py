...
from .utils import hash_to_int, int_to_bytes, sha256_hex
...
class Ciphertext:
    ...
    def context_bytes(self) -> bytes:
        return b"|".join(
            [
                ...
                self.plaintext_commitment.encode("ascii"),
            ]
        )
    ...
    def from_dict(value: dict[str, str]) -> "Ciphertext":
        return Ciphertext(
            ...
            plaintext_commitment=str(value["plaintext_commitment"]),
        )
...
def _plaintext_commitment(buyer: str, resource: str, order_id: str, plaintext: int) -> str:
    return sha256_hex(
        b"|".join(
            (
...
def encrypt_capability(
    ...
    """Encrypt a high-entropy capability and expose only a binding commitment.

    The seed parameter exists for isolated unit tests. The experiment path never
    supplies it and therefore obtains both token material and ElGamal randomness
    from the operating system CSPRNG. Neither the token nor a reproducible seed
    is serialized into the request or evidence.
    """
    ...
    return Ciphertext(
        ...
        resource=resource,
        order_id=order_id,
        plaintext_commitment=_plaintext_commitment(buyer, resource, order_id, plaintext),
    )
...
def _proof_witness(share: int, operator_id: int, ciphertext: Ciphertext, epoch: int) -> int:
    """The Chaum--Pedersen nonce, derived inside the operator.

    It is a deterministic function of the operator's secret share and of every
    field that can change the Fiat--Shamir challenge --- the ciphertext, its
    buyer/resource/order context, the operator identity, and the epoch --- in the
    style of RFC 6979. The invariant we need is: the nonce binds a superset of the
    challenge's free inputs. Two properties then hold, and both fail if the
    requester is allowed to choose the nonce:

    * It is unpredictable to anyone without the share, so it is a sound witness.
    * It is distinct whenever the challenge could differ, so a witness is shared
      across two distinct challenges only with negligible hash-to-scalar collision
      probability. Reusing one witness across two distinct statements would expose
      the share via ``s = (z1 - z2)(e1 - e2)^{-1}``; binding the witness to the full
      statement makes that reuse cryptographically infeasible. In particular a
      second proof by the same operator over the same ``(c1, c2)`` but a different
      order or epoch uses a different witness.

    The requester therefore has no input to it at all.
    """
...
def create_partial(
    ...
    witness = _proof_witness(share, operator_id, ciphertext, epoch)
    a1 = pow(G, witness, P)
    a2 = pow(ciphertext.c1, witness, P)
    challenge = _proof_challenge(operator_id, public_share, ciphertext, epoch, partial, a1, a2)
...
def verify_partial(public_share: int, ciphertext: Ciphertext, response: PartialDecryption, epoch: int) -> bool:
...
def verify_threshold_result(ciphertext: Ciphertext, plaintext: int) -> bool:
    return _plaintext_commitment(ciphertext.buyer, ciphertext.resource, ciphertext.order_id, plaintext) == ciphertext.plaintext_commitment
