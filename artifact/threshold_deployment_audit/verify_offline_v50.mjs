...
const frontier = spawnSync(process.execPath, [path.join(ROOT, "scripts", "evidence_frontier_v50.mjs")], { cwd: ROOT, encoding: "utf8" });
if (frontier.status !== 0 || !frontier.stdout.includes("SCOPE_BOUND_COUNTERMODEL_FRONTIER=PASS")) {
  throw new Error(`countermodel frontier failed\n${frontier.stdout}\n${frontier.stderr}`);
}
const frontierTests = spawnSync(process.execPath, [path.join(ROOT, "test_evidence_frontier_v50.mjs")], { cwd: ROOT, encoding: "utf8" });
if (frontierTests.status !== 0 || !frontierTests.stdout.includes("EVIDENCE_FRONTIER_V50_TESTS=PASS")) {
  throw new Error(`countermodel frontier tests failed\n${frontierTests.stdout}\n${frontierTests.stderr}`);
}
...
console.log(frontier.stdout.trim());
console.log(frontierTests.stdout.trim());
console.log(`CANONICAL_SHA256=${sha(canonical)}`);
console.log(`MONOTONICITY_COMPARISONS=${comparisons}`);
console.log("FINITE_LTS_EVIDENCE_AND_COUNTERMODEL_FRONTIER_OFFLINE=PASS");
