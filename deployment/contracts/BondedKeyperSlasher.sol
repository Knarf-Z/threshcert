...
contract BondedKeyperSlasher is EIP712, Ownable, ReentrancyGuard {
    ...
    constructor(address initialOwner, address evidenceVerifier, address payable penaltyTreasury)
        EIP712("BondedKeyperSlasher", "1")
    ...
    function registerMember(uint8 memberIndex, address signer) external payable onlyOwner {
        ...
        if (msg.value == 0 || msg.value > type(uint96).max) revert InvalidBond();
