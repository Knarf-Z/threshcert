...
target="$UPSTREAM/rolling-shutter/tools/fc-evidence-exporter"
...
docker run --rm \
  ...
  --mount "type=volume,src=fc-go-mod-cache,dst=/go/pkg/mod" \
  --mount "type=volume,src=fc-go-build-cache,dst=/root/.cache/go-build" \
  --workdir /src \
  golang:1.23.8 \
  sh -c '
    set -eu
    go test ./tools/fc-evidence-exporter
    go build -trimpath \
      -o /out/fc-evidence-exporter \
      ./tools/fc-evidence-exporter
  '

chmod +x "$ROOT/bin/fc-evidence-exporter" 2>/dev/null || true
echo "built $ROOT/bin/fc-evidence-exporter using Docker"