...
/// Submits slashing evidence against the parallel, additive
...
/// than an edit of submit-evidence.ts / BondedKeyperSlasher.sol).
...
const jobPath = resolve(
  process.env.JOB_FILE?.trim() ?? `results/job-${networkName}-rewarded.json`,
);
...
const verifierSignature = await verifier.signTypedData({
  domain: evidenceDomain(chainId, deployment.contract, "BondedKeyperSlasherRewarded"),
...
const netSubmitterGainWei = BigInt(deployment.callerRewardWei) - gasCostWei;

const result = {
  schema: "fc-slashing-result-rewarded-v1",
  ...
  bondForfeitedWei: deployment.bondWeiPerMember,
  callerRewardWei: deployment.callerRewardWei,
  treasuryAmountWei: (
    BigInt(deployment.bondWeiPerMember) - BigInt(deployment.callerRewardWei)
  ).toString(),
  caller: getAddress(submitter.account.address),
  gasCostWei: gasCostWei.toString(),
  netSubmitterGainWei: netSubmitterGainWei.toString(),
  rewardCoversGasCost: netSubmitterGainWei > 0n,
  transaction: transactionRecord(receipt),
};
const output = process.env.SLASHING_OUTPUT?.trim()
  ?? `results/slashing-${networkName}-rewarded.json`;
...
console.log(`gas_cost_wei=${gasCostWei}`);
console.log(`net_submitter_gain_wei=${netSubmitterGainWei}`);
console.log(`reward_covers_gas_cost=${netSubmitterGainWei > 0n}`);
