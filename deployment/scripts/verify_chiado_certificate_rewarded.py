#!/usr/bin/env python3
"""Build or verify the machine-readable certificate for the recorded rewarded
Chiado run.

This is a parallel, additive script -- `verify_chiado_certificate.py` binds
`contracts/BondedKeyperSlasher.sol` and `scripts/verify-chiado-live.ts` into
an already-recorded historical certificate, so it is left untouched. This
script instead certifies the separate `BondedKeyperSlasherRewarded` run,
recorded independently in `results/deployment-chiado-rewarded.json`,
`results/job-chiado-rewarded.json`, and `results/slashing-chiado-rewarded.json`.

The certificate is intentionally narrow, exactly like its un-rewarded
counterpart: it binds the recorded evidence, public Chiado transactions, and
exact four-smallest-bonds calculation, plus (new here) the claim that the
caller reward actually covers the caller's own gas cost. It does not turn
this controlled single-host testnet run into a production Shutter resistance
or activation certificate.
"""
...
CERTIFICATE_PATH = ROOT / "certificates" / "chiado-execution-certificate-rewarded.json"
SCHEMA = "fc-chiado-execution-certificate-rewarded-v1"
DOMAIN_SEPARATOR = b"fc-chiado-execution-certificate-rewarded-v1\x00"
SOURCE_PATHS = (
    "contracts/BondedKeyperSlasherRewarded.sol",
    "evidence/shutter-evidence.json",
    "results/deployment-chiado-rewarded.json",
    "results/job-chiado-rewarded.json",
    "results/slashing-chiado-rewarded.json",
    "rolling-shutter/evidence-exporter/main.go",
    "rolling-shutter/evidence-exporter/main_test.go",
    "runtime/keyper-set.json",
    "scripts/submit-evidence-rewarded.ts",
)
...
def build_certificate() -> dict[str, Any]:
    deployment = load_json("results/deployment-chiado-rewarded.json")
    job = load_json("results/job-chiado-rewarded.json")
    evidence = load_json("evidence/shutter-evidence.json")
    slashing = load_json("results/slashing-chiado-rewarded.json")
    keyper_set = load_json("runtime/keyper-set.json")

    require(
        deployment.get("schema") == "fc-bonded-keyper-deployment-rewarded-v1",
    ...
    require(slashing.get("schema") == "fc-slashing-result-rewarded-v1", "bad slashing schema")
    ...
    caller_reward = integer(deployment.get("callerRewardWei"), "callerRewardWei")
    require(bond > caller_reward, "bond does not exceed the caller reward")
    ...
    bond_forfeited = integer(slashing.get("bondForfeitedWei"), "bondForfeitedWei")
    caller_reward_paid = integer(slashing.get("callerRewardWei"), "slashing callerRewardWei")
    treasury_amount = integer(slashing.get("treasuryAmountWei"), "treasuryAmountWei")
    require(bond_forfeited == bond, "forfeited bond does not match the registered bond")
    require(caller_reward_paid == caller_reward, "paid caller reward does not match the deployment's")
    require(bond_forfeited == caller_reward_paid + treasury_amount, "reward split does not sum to the forfeited bond")

    slashing_transaction = slashing["transaction"]
    gas_cost = integer(slashing.get("gasCostWei"), "gasCostWei")
    computed_gas_cost = integer(slashing_transaction.get("gasUsed"), "gasUsed") * integer(
        slashing_transaction.get("effectiveGasPrice"), "effectiveGasPrice"
    )
    require(gas_cost == computed_gas_cost, "recorded gas cost does not match gasUsed * effectiveGasPrice")
    net_gain = integer(slashing.get("netSubmitterGainWei"), "netSubmitterGainWei")
    require(net_gain == caller_reward_paid - gas_cost, "net submitter gain does not match reward minus gas cost")
    reward_covers_gas = slashing.get("rewardCoversGasCost")
    require(isinstance(reward_covers_gas, bool), "rewardCoversGasCost must be boolean")
    require(reward_covers_gas == (net_gain > 0), "rewardCoversGasCost does not match the computed net gain")
    require(reward_covers_gas is True, "recorded run does not show the reward covering the caller's gas cost")

    source_digests = {relative: sha256_file(relative) for relative in SOURCE_PATHS}
    payload: dict[str, Any] = {
        "schema": SCHEMA,
        "certificateClass": "controlled-public-testnet-threshold-cover-with-covered-enforcement-reward",
        "claim": {
            "status": "POSITIVE_WITHIN_RECORDED_TESTNET_MECHANISM",
            "statement": (
                "At the recorded rewarded-Chiado state, the instrumented 4-of-7 contract's "
                f"sum-of-four-smallest-live-bonds value is {before_value} wei, and the recorded "
                "slashing transaction paid its own submitter a caller reward that exceeds the "
                f"transaction's own gas cost by {net_gain} wei -- enforcement is rational for an "
        ...
        "scope": {
            ...
            "mechanism": (
                "verifier-attested early-share slashing with on-chain bond transfer, "
                "split between a funded penalty treasury and the transaction's own submitter"
            ),
            "certificateInput": (
                "remaining on-chain testnet bond balances under the covered "
                "verifier-gated enforcement path, plus the caller-reward-vs-gas-cost margin"
            ),
        ...
        "enforcementIncentive": {
            "callerRewardWei": str(caller_reward_paid),
            "treasuryAmountWei": str(treasury_amount),
            "gasCostWei": str(gas_cost),
            "netSubmitterGainWei": str(net_gain),
            "rewardCoversGasCost": True,
        },
        "observedEvidence": {
        ...
        "verification": {
            "offlineCommand": "python scripts/verify_chiado_certificate_rewarded.py",
...
def verify_stored_certificate(expected: dict[str, Any]) -> None:
    ...
    require(actual == expected, "stored rewarded Chiado execution certificate is stale or inconsistent")


def print_summary(certificate: dict[str, Any]) -> None:
    evidence = certificate["observedEvidence"]
    value = certificate["certificate"]
    incentive = certificate["enforcementIncentive"]
    ...
    print(f"caller_reward_wei={incentive['callerRewardWei']}")
    print(f"gas_cost_wei={incentive['gasCostWei']}")
    print(f"net_submitter_gain_wei={incentive['netSubmitterGainWei']}")
    print(f"reward_covers_gas_cost={incentive['rewardCoversGasCost']}")
    print("offline_evidence_binding=PASS")
    print("production_shutter_certificate=NOT_CERTIFIED")
    print("chiado_rewarded_execution_certificate=PASS")
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"chiado_rewarded_execution_certificate=FAIL: {exc}") from exc
