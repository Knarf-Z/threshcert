...
- The mechanism-stress instance in `reproduce_paper_numbers.py` is the
  appendix's fixed-uniform-cost construction at zero liquidity: every member
  floor is one and the checked costs are `k+2` sequential versus `2`
  package/static threshold cover.
